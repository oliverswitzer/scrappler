...
if (!email || !password) throw new Error('WeWork EMAIL and PASSWORD must be set in .env');
...
export default class WeWorkScraper extends BaseScraper {
  async login() {
    ...
    await this.page.goto('https://members.wework.com/workplaceone/content2/login/welcome');
  ...
  async bookDesk() {
    ...
    const res = await this.page.evaluate(async () => {
      // @ts-ignore
      document.querySelectorAll('.book-desk-button')[0].dispatchEvent(new Event('click'));
    });
