...
import { Listing, NEIGHBORHOOD_ENUM_TO_STRING_MAP, Neighborhood } from 'rb-shared';

export const Listings = ({ listings }: { listings: Listing[] }) => {
  return <Grid data-listings={JSON.stringify(listings)} container spacing={4} style={{ padding: 24 }}>
    {listings.map(listing => (
      <Grid item xs={12} sm={12} md={4} key={listing.id}>
        <Card>
          <Carousel autoPlay={false}>
            {listing.images.map((imgSrc: string, i: number) => (
              <Paper className="overflow-clip h-[250px]" key={`${listing.id}-img-${i}`} elevation={0}>
                <a target="_blank" rel="noopener" href={listing.url.toString()}>
                  <img className="w-full h-full object-cover" src={imgSrc} alt={`Image of ${listing.address}`} />
                </a>
              </Paper>
            ))}
          </Carousel>
